...
BASE_URL="https://raw.githubusercontent.com/GTPSHAX/luckyproxy-bin/c516577b36b76b39b65e874a49ed93d56498efa1"
...
if [ "$DEBUG" -eq 1 ]; then
  ...
  echo -e "${YELLOW}[DEMO] Mode debug aktif, akan mengunduh ${BIN_NAME}${NC}"
else
  BIN_NAME="LuckyProxy"
...
download_resource() {
  ...
  if [ ! -f "$destination" ]; then
    ...
    echo -e "${CYAN}[DEMO] Mengunduh $resource_path ...${NC}"
    curl -fsSL -o "$destination" "$BASE_URL/beta/resources/$resource_path" || {
      echo -e "${RED}[ERROR] Gagal mengunduh $resource_path${NC}" >&2
      exit 1
    }
    echo -e "${GREEN}[DEMO] Disimpan di: $(pwd)/$destination${NC}"
  else
    echo -e "${YELLOW}[DEMO] $resource_path sudah ada, melewati download.${NC}"
  fi
}
...
if [ "$DEBUG" -eq 1 ]; then
  echo -e "${GREEN}[DEMO] Selesai, sekarang Anda bisa menjalankan ${CYAN}LD_LIBRARY_PATH=$RESOURCE_DIR ./$BIN_NAME${NC}"
else
  echo -e "${GREEN}[DEMO] Selesai, sekarang Anda bisa menjalankan ${CYAN}./$BIN_NAME${NC}"
fi
