...
if [ "$DEBUG" -eq 1 ]; then
  ...
  echo -e "${YELLOW}[DEMO] Debug mode enabled, will download ${BIN_NAME}${NC}"
else
  BIN_NAME="LuckyProxy"
...
download_resource() {
  ...
  if [ ! -f "$destination" ]; then
    ...
    echo -e "${CYAN}[DEMO] Downloading $resource_path ...${NC}"
    curl -fsSL -o "$destination" "$BASE_URL/beta/resources/$resource_path" || {
      echo -e "${RED}[ERROR] Failed to download $resource_path${NC}" >&2
      exit 1
    }
    echo -e "${GREEN}[DEMO] Saved: $(pwd)/$destination${NC}"
  else
    echo -e "${YELLOW}[DEMO] $resource_path exists, skipping.${NC}"
  fi
}
...
if [ "$DEBUG" -eq 1 ]; then
  echo -e "${GREEN}[DEMO] Done, now you can run ${CYAN}LD_LIBRARY_PATH=$RESOURCE_DIR ./$BIN_NAME${NC}"
else
  echo -e "${GREEN}[DEMO] Done, now you can run ${CYAN}./$BIN_NAME${NC}"
fi
